...
describe("TenantFactory", function () {
  ...
  async function deployTenantFactoryFixture() {
    ...
    const erc20 = await hre.viem.deployContract("BasicERC20", [tenantOwner.account.address, "Test ERC20", "TST"], { client: { wallet: tenantOwner } });
    const sbt = await hre.viem.deployContract("ERC20NonTransferable", [tenantOwner.account.address, "Test SBT", "SBT"], { client: { wallet: tenantOwner } });
    ...
    return { deployer, tenantFactory, tenantOwner, nftOwner, newNftOwner, publicClient, erc20, sbt, nft, erc20Owner };
  }
...
it("should verify compatibility with reused ERC20 and SBT contracts", async function () {
  ...
  it("settleAll should proceed even if one tenant fails", async function () {
    const { tenantFactory, deployer, publicClient, erc20, nftOwner, nft } = await loadFixture(deployTenantFactoryFixture);
    ...
    const tx1 = await tenantFactory.write.createTenant(["Tenant1", 1, defaultAddress, payoutPeriod], { account: tenantOwner1.account });
    const receipt1 = await publicClient.waitForTransactionReceipt({ hash: tx1 });
    const tenant1Address = parseEventLogs({ logs: receipt1.logs, abi: hre.artifacts.readArtifactSync("TenantFactory").abi })
        .find((log) => log.eventName === "TenantCreated")?.args.tenantAddress;

    const tx2 = await tenantFactory.write.createTenant(["Tenant2", 1, defaultAddress, payoutPeriod], { account: tenantOwner2.account });
    const receipt2 = await publicClient.waitForTransactionReceipt({ hash: tx2 });
    const tenant2Address = parseEventLogs({ logs: receipt2.logs, abi: hre.artifacts.readArtifactSync("TenantFactory").abi })
        .find((log) => log.eventName === "TenantCreated")?.args.tenantAddress;
    ...
    await tenant1.write.mint([initialTreasuryBalance], { account: tenantOwner1.account });
    await tenant2.write.mint([insufficientBalance], { account: tenantOwner2.account });
    ...
    await tenant1.write.record([reqID1, amountToSettle, BigInt(1), nft.address, BigInt(0)], { account: tenantOwner1.account });
    await tenant2.write.record([reqID2, amountToSettle, BigInt(1), nft.address, BigInt(0)], { account: tenantOwner2.account });
