...
import { parseEventLogs, getAddress, parseEther, formatEther } from "viem";
...
describe("Tenant", function () {
    ...
    async function deployTenantWithFactory() {
        ...
        return { tenantFactory, tenantOwner, nftOwner, newNftOwner, publicClient, erc20, sbt, nft, erc20Owner };
    }
    ...
    it("should only allow owner to control treasury funds", async function () {
        ...
        const tx = await tenantFactory.write.createTenant(
            ["Tenant Controlled ERC20", 1, defaultAddress, payoutPeriod],
            { account: tenantOwner.account }
        );
        ...
        await expect(tenant.write.setCurrencyAddress([defaultAddress], { account: nftOwner.account })).to.be.revertedWith('Not authorized');
    });
    ...
    it("should settle UTXRs (Tenant with ETH currency)", async function () {
        ...
        const initialNftOwnerBalance = await publicClient.getBalance({ address: nftOwner.account.address });
        ...
        await tenantOwner.sendTransaction({ to: tenantAddress!, value: initialTreasuryBalance });
        ...
        expect(await publicClient.getBalance({ address: nftOwner.account.address })).to.equal(initialNftOwnerBalance + amount);
    })

    it("should settle UTXRs (Tenant with ERC20 currency)", async function () {
        const { tenantFactory, tenantOwner, publicClient, erc20, nftOwner, nft } = await loadFixture(deployTenantWithFactory);

        const initialTreasuryBalance = BigInt(100000);

        const tx = await tenantFactory.write.createTenant(
            ["Settle Tenant", 1, defaultAddress, payoutPeriod],
            { account: tenantOwner.account }
        );
        ...
        await tenant.write.mint([initialTreasuryBalance], { account: tenantOwner.account });
    ...
    it("should settle UTXRs (Tenant with ERC20 currency), with pre-deployed ERC20 contract", async function () {
      ...
      await erc20.write.mint([tenantAddress!, initialTreasuryBalance], { account: tenantOwner.account });
    ...
    it("should settle UTXRs (Tenant with SBT currency)", async function () {
        ...
        const tx = await tenantFactory.write.createTenant(
            ["Settle Tenant", 2, defaultAddress, payoutPeriod],
            { account: tenantOwner.account }
        );
        const receipt = await publicClient.waitForTransactionReceipt({ hash: tx });
        const logs = parseEventLogs({ logs: receipt.logs, abi: hre.artifacts.readArtifactSync("TenantFactory").abi });
